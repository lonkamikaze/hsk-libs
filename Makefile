...
_LOCAL_MK!=	test -f Makefile.local || touch Makefile.local ; echo 1
...
_GEN!=		mkdir -p ${GENDIR} ; echo 1

# Configure SDCC.
_SDCC_MK:=	$(shell env CC="${CC}" sh scripts/sdcc.sh ${CONFDIR}/sdcc > ${GENDIR}/sdcc.mk)
_SDCC_MK!=	env CC="${CC}" sh scripts/sdcc.sh ${CONFDIR}/sdcc > ${GENDIR}/sdcc.mk ; echo 1
...
_DBC_MK!=	sh scripts/dbc.sh ${CANPROJDIR}/ > ${GENDIR}/dbc.mk ; echo 1
...
_DBC_MK!=	${MAKE} DBCDIR=${DBCDIR} -f ${GENDIR}/dbc.mk dbc 1>&2 ; echo 1

# Generate build
_BUILD_MK:=	$(shell sh scripts/build.sh src/ ${INCDIR}/ ${GENDIR}/ > ${GENDIR}/build.mk)
_BUILD_MK!=	sh scripts/build.sh src/ ${INCDIR}/ ${GENDIR}/ > ${GENDIR}/build.mk ; echo 1
